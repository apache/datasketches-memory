...
public class SpecificLeafTest {
  ...
  public void checkByteBufferLeaves() {
    ...
    assertTrue(bsi.hasByteBuffer());
    assertTrue(ResourceImpl.isNativeOrder(typeId));
  ...
  public void checkDirectLeaves()  {
    ...
    try (WritableMemory wmem = WritableMemory.allocateDirect(bytes)) {
      assertTrue(((ResourceImpl)wmem).isDirect());
  ...
  public void checkHeapLeaves() {
    ...
    assertTrue(((ResourceImpl)buf2).isRegionView());
    assertTrue(((ResourceImpl)buf3).isDuplicate());
    assertTrue(((ResourceImpl)buf4).isDuplicate());
  }
  ...
  public void checkMapLeaves() throws IOException {
    ...
    try (WritableMemory mem = WritableMemory.writableMap(file, 0L, bytes, ByteOrder.nativeOrder())) {
      assertTrue(((ResourceImpl)mem).isMapped());
  ...
  private static void checkCrossLeafTypeIds(Memory mem) {
    ...
    assertTrue(ResourceImpl.isBuffer(((ResourceImpl)buf1).getTypeId()));
    assertTrue(buf1.isReadOnly());

    Buffer buf2 = buf1.duplicate();
    assertTrue(((ResourceImpl)buf2).isRegionView());
    assertTrue(ResourceImpl.isBuffer(((ResourceImpl)buf2).getTypeId()));
    assertTrue(((ResourceImpl)buf2).isDuplicate());
    assertTrue(buf2.isReadOnly());

    Memory mem2 = buf1.asMemory(); //
    assertTrue(((ResourceImpl)mem2).isRegionView());
    assertFalse(ResourceImpl.isBuffer(((ResourceImpl)mem2).getTypeId()));
    ...
    assertTrue(ResourceImpl.isBuffer(((ResourceImpl)buf3).getTypeId()));
    assertTrue(((ResourceImpl)buf3).isDuplicate());
    ...
    assertFalse(ResourceImpl.isBuffer(((ResourceImpl)mem3).getTypeId()));
