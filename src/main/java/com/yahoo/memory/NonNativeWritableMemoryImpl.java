...
final class NonNativeWritableMemoryImpl extends BaseWritableMemoryImpl {
  ...
  BaseWritableBufferImpl asWritableBufferImpl(final boolean localReadOnly) {
    ...
    if (getCapacity() == 0) {
      ...
      wbuf = new NonNativeWritableBufferImpl(getUnsafeObject(), getNativeBaseOffset(),
          getRegionOffset(), getCapacity(), isReadOnly(), getByteBuffer(), getValid(), this);
  ...
  public void getCharArray(final long offsetBytes, final char[] dstArray, final int dstOffsetChars,
    ...
    CompareAndCopy.getNonNativeChars(getUnsafeObject(), getCumulativeOffset(), offsetBytes, copyBytes,
