...
class WritableBufferImpl extends WritableBuffer {
  ...
  final static WritableBufferImpl ZERO_SIZE_ARRAY_BUFFER;

  static {
    ZERO_SIZE_ARRAY_BUFFER = new WritableBufferImpl(new ResourceState(new byte[0], Prim.BYTE, 0));
  }
  ...
  @Override
  public void checkValidAndBounds(final long offsetBytes, final long length) {
    state.checkValid();
    checkBounds(offsetBytes, length, capacity);
  }
